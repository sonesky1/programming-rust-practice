...
fn test_parse_pair() {
    ...
    assert_eq!(pase_pair::<f64>("0.5x1.5", 'x'), Some((0.5, 1.5)));
}
...
fn main() {
    println! {"pair {:?}", parse_pair::<i32>("10,155", ',')};
    let args: Vec<String> = env::args().collect();

    if args.len() != 5 {
        eprintln!("Usage: {} FILE PIXELS UPPERLEFT LOWERRIGHT", args[0]);
        eprintln!(
            "exmaple: {} mandel.png 100x750 -1.20, 0.35 -1,0.20",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    //remove this as now we will make concurrent
    ...
    write_image(&args[1], &pixels, bounds).expect("eror writing PNG file");
}
